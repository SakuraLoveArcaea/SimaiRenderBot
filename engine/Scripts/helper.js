...
export class SimaiLogicControler {
    ...
    get({
        ...
        // 穿透模式（tap / hold 不擊打、直接飛出判定線）要留夠時間讓音符淡出，
        // 不然會在還看得見的時候就被剔除。只有 tap / hold 需要這個放寬。
        const tailKeepTime = settings.noteEndBehavior === 'through'
            ? Math.max(effectDecayTime, settings.noteEndFadeTime ?? 0.3)
            : effectDecayTime;
            ...
            const isVisible =
                (noteType === "slide" ? t >= middleDistance :
                    noteType === "touch" ? touchT >= -1 :
                        t >= -1)
                && -noteT <= skipT + (note.isHanabi ? hanabiEffectDecayTime :
                    (noteType === 'slide' ? 0 :
                        ((noteType === 'tap' && !note.isStar) || noteType === 'hold' ? tailKeepTime : effectDecayTime)));
