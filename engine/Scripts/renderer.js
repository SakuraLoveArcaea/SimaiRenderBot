...
export class SimaiRenderer {
    ...
    /**
     * @param {number} tailT 音符「尾端」通過判定線的相對時間，<= 0 表示已經通過。
     *                       tap / star / touch 直接用 noteT；hold / touch hold 用 noteT + holdDuration。
     * @returns {number} 這一格要套用的透明度；0 代表已完全淡出，呼叫端應直接跳過繪製。
     */
    passThroughAlpha(tailT) {
        const fade = this.settings.noteEndFadeTime ?? 0.3;
        if (fade <= 0) return 0;
        return clamp(1 + tailT / fade, 0, 1);
    }
    ...
    drawTap(s) {
        ...
        if (noteT <= 0) {
            ...
            // 穿透模式：getNoteTransform 在 noteT < 0 時本來就會給出 t > 1，
            // 音符會自己繼續往外飛，這裡只要負責淡出。
            fadeAlpha = this.passThroughAlpha(noteT);
        ...
        ctx.globalAlpha = currentScale * fadeAlpha;
    ...
    drawStar(s) {
        ...
        // star 不套用穿透：slide 的頭就是 star，讓它飛出去會跟軌道上開始跑的那顆星星並存
        if (noteT <= 0) {
            ctx.save();
            ctx.translate(posInfo.x, posInfo.y);
            this.simpleHitEffect(noteT);
