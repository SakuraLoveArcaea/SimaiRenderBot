...
var ZodFunction = class _ZodFunction extends ZodType {
  ...
  _parse(input) {
    ...
    const params2 = { errorMap: ctx2.common.contextualErrorMap };
    const fn = ctx2.data;
  ...
  (function(module) {
    var bigInt2 = function(undefined$1) {
      ...
      function randBetween(a, b, rng2) {
        ...
        var digits = toBase(range2, BASE2).value;
...
function fromHexReverseArray(hexValues, start, size2) {
  let value = 0;
  ...
  if (slide) {
    if (str.includes("##")) {
      ...
      if (parts.length === 3) {
        ...
        return { time: 240 / overrideBpm * (parseFloat(beat) / parseFloat(time)), delay };
        ...
        if (parts[1].includes(":")) {
          const [time2, beat] = parts[1].split(":");
          if (isNaN(beat) || parseFloat(time2) < 0 || parseFloat(beat) < 0) {
            console.warn("Invalid time or beat value in slide note:", str);
            return { time: -1, delay: -1 };
          }
          return { time: 240 / bpm * (parseFloat(beat) / parseFloat(time2)), delay };
        }
      ...
      return { time, delay: 60 / overrideBpm };
    }
  } else {
    if (str.startsWith("#")) {
...
var PathRecorder = class {
  ...
  getPointAt(t) {
    ...
    const seg = this.segments.find((s) => targetLen >= s.cumLength && targetLen <= s.cumLength + s.length) || this.segments[this.segments.length - 1];
...
var noteRefPos = Array.from({ length: 8 }, (_, i) => {
  const a = (i - 1.5) * Math.PI / 4;
  return {
...
var visualNoteRefPos = Array.from({ length: 8 }, (_, i) => {
  return {
...
var AudioManager = class {
  constructor() {
    this.globalGain = 0.65;
    this.bgmVolume = 0.8;
    this.sfxMasterVolume = 0.5;
    this.reinitContext();
    this.bufferMap = /* @__PURE__ */ new Map();
    this.playingSources = /* @__PURE__ */ new Map();
    this.soundQueue = [];
    this.lastQueuedTimes = /* @__PURE__ */ new Map();
    ...
    this.activeLongSounds = /* @__PURE__ */ new Map();
    this.loopPoints = {
  ...
  reinitContext() {
    ...
    if (typeof window !== "undefined") {
      ...
      if (now - window.__lastAudioReinitTime < 5e3) {
        return;
      }
    ...
    try {
      ...
      this.longSoundCompressor.attack.setValueAtTime(5e-3, nowTime);
  ...
  ensureContextSync() {
    ...
    if (this.ctx && this.ctx.state === "suspended") {
      ...
      if (typeof window !== "undefined") {
        ...
        if (now - window.__lastAudioResumeAttemptTime > 3e3) {
        ...
        if (!this.lastResumeAttemptTime || now - this.lastResumeAttemptTime > 3e3) {
  ...
  */
  ...
  startLongSound(id, key, offset = 0) {
    const buffer = this.bufferMap.get(key);
    const loop2 = this.loopPoints[key];
    ...
    if (loop2) {
      source.loop = true;
      source.loopStart = loop2.start;
      source.loopEnd = loop2.end;
      if (offset >= loop2.end) {
        const loopDuration = loop2.end - loop2.start;
        const timeInsideLoop = (offset - loop2.end) % loopDuration;
        startTimeWithinBuffer = loop2.start + timeInsideLoop;
      }
    } else {
  ...
  stopLongSound(id) {
    if (this.activeLongSounds.has(id)) {
      ...
      gainNode.gain.exponentialRampToValueAtTime(1e-3, this.ctx.currentTime + 0.05);
...
var touchPathConfigs = {
  A: { points: [[0.31, 1], [0.31, 0.65], [0.15, 0.6]] },
...
for (let i = 1; i <= 8; i++) {
  ...
  ["A", "B", "D", "E"].forEach((type) => {
    ...
    const config2 = touchPathConfigs[type];
    const len = config2.points.length;
    const base = baseAngles[type];
    for (let j = 0; j < len * 2; j++) {
      let [angleOffset, radiusMult] = j < len ? config2.points[j] : config2.points[len - 1 - (j - len)];
...
var _cEl = document.getElementById("canvasContainer");
try {
...
var activeDebug = () => {
  ...
  debugInfoEl.style.cursor = "move";
  debugInfoEl.style.userSelect = "none";
...
var formatWarnArg = (arg) => {
  if (typeof arg === "string") return arg;
  try {
    if (isObject(arg)) {
      if (arg.errpos !== void 0) {
...
var _sp = [];
function simaiDecode(data = "", baseOffset = true) {
  warns = [];
  warnpos = [];
  const raw = data.replace(/\|\|.*$/gm, "").replace(/\s+/g, "");
  ...
  const tags = [];
  const tempNotes = [];
  let firstBpm = null, endTime = 0, nowTime = 0, nowBpm = 60, nowSplit = 4, hispeed = 1, overrideSplitTime = null, noteCommaIndex = 0, indexToTime = [];
  ...
  for (let e of splitParts) {
    ...
    ;
    {
      ...
      if (e.includes("`")) {
        ...
        notesToProcess = subNotes.map((raw2, i) => ({ raw: raw2, time: nowTime + i * 1e-3 }));
      } else {
        notesToProcess = [{ raw: e, time: nowTime }];
      }
      notesToProcess.forEach(({ raw: raw2, time }) => {
        let props = [];
        while (raw2.startsWith("<")) {
          ...
          raw2 = raw2.slice(match[0].length);
        }
        while (raw2.endsWith(">")) {
          const match = raw2.match(/<([^<>]*)>$/);
          if (!match) break;
          props.push(match[1].trim());
          raw2 = raw2.slice(0, -match[0].length);
        }
        if (props.length === 0) props = null;
        if (props) {
          const prop2 = props[props.length - 1];
          if (prop2.startsWith("HS*")) {
            const hispeedValue = parseFloat(prop2.slice(3));
            if (!isNaN(hispeedValue)) {
              hispeed = hispeedValue;
            } else {
              pushWarn("Invalid hispeed value in property:", { errpos: noteCommaIndex });
        ...
        const splitr = (() => {
          if (raw2.includes("/")) {
            return raw2.split("/").map((s) => s.trim());
          } else {
            return [raw2.trim()];
          }
        })();
        ...
        parts.forEach((noteStr) => {
          ...
          const posMatch = noteStr.match(/^\d+/);
          const touchMatch = noteStr.match(/^([ABCDE])(\d+)|C/);
          ...
          if (slideMatch && !noteStr.includes("h")) {
            ...
            for (let i = 0; i < slideParts.length; i++) {
              ...
              noteObj.isStar = true;
              const p = residue.split(/((?:pp)|(?:qq)|[-<>^vpqszVw])/g).filter((_, i2) => i2 % 2 === 0);
              if (p[0].includes("b")) {
                noteObj.isBreak = true;
                p[0] = p[0].replace(/b/g, "");
                breakCounts++;
              ...
              if (isSlideMine) {
                p.forEach((c, i2) => {
                  ...
                  p[i2] = p[i2].replace(/m/g, "");
                });
              }
              let d = 0, dlay = 0;
              {
                let error = false;
                timeValues.forEach((t, i2) => {
              ...
              const segments = slidePartMatch.map((type, i2) => {
                const head = i2 === 0 ? noteObj.pos : parseInt(p[i2].slice(-1));
                const part = p[i2 + 1];
                const end = parseInt(part.slice(-1));
                const mid = part.length > 1 ? parseInt(part.slice(-2, -1)) : void 0;
                if ([head, end].some((v) => isNaN(v) || v < 1 || v > 8)) return null;
                if (type === "V" && mid === void 0 || mid !== void 0 && (isNaN(mid) || mid < 1 || mid > 8)) return null;
...
function getSlidePath(start, end, type, mid = null) {
  ...
  switch (type) {
    ...
    case "V": {
      ...
      const m = (mid - end + 8) % 8;
      if (s !== 2 && s !== 6 || e || mid === end || start === mid || mid === start || s === 2 && !(m >= 2 && m <= 5) || s === 6 && !(m >= 3 && m <= 6)) {
        illegal = true;
      }
    ...
    case "pp": {
      ...
      r.arc(cir.x, cir.y, innerCirleBase * 0.466, startInfo.rot - Math.PI, endInfo.rot + Math.PI * ((c == 0) * -0.3 + (c == 1) * -0.35 + (c == 2) * -0.2 + (c == 4) * 0.02 + (c == 6) * -0.15 + (c == 7) * -0.2), true, end > start && (end - start + 8) % 8 >= 3 || start > end && (end - start + 8) % 8 == 3);
      r.lineTo(endInfo.x, endInfo.y);
      break;
    }
    case "qq": {
      const c3 = (start - end + 8) % 8;
      const cir = {
      ...
      r.arc(cir.x, cir.y, innerCirleBase * 0.466, startInfo.rot, endInfo.rot + Math.PI * (-1 + (c3 == 0) * 0.3 + (c3 == 1) * 0.35 + (c3 == 2) * 0.2 + (c3 == 4) * -0.02 + (c3 == 6) * 0.15 + (c3 == 7) * 0.2), false, start > end && (start - end + 8) % 8 >= 3 || end > start && (start - end + 8) % 8 == 3);
      r.lineTo(endInfo.x, endInfo.y);
      break;
    }
    case "s":
...
var statusEl = $("status");
var songTitleEl = $("songTitle");
var metaLineEl = $("metaLine");
...
function loop(ts) {
  if (!playing) return;
  const dt = Math.min(100, ts - lastTs) / 1e3;
  lastTs = ts;
  realTime += dt * speed;
